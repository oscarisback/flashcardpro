...
# This ensures data persists between Streamlit Cloud restarts
...
def sync_data_from_github():
    ...
    try:
        ...
        file = repo.get_contents("flashcard_data.json")
        ...
        # Silently fail - will use local data if available
        return None

def sync_data_to_github(data):
    ...
    try:
        ...
        try:
            ...
            repo.update_file(
                "flashcard_data.json",
                f"Auto-sync: Update flashcard data",
            ...
            repo.create_file(
                "flashcard_data.json",
                "Initial commit: Create flashcard data",
                json.dumps(data, indent=4, ensure_ascii=False)
            )
        
        return True
    except Exception as e:
        # Silently fail - local save is still good
        return False

def load_data():
    # Try to sync from GitHub first (Streamlit Cloud persistence)
    github_data = sync_data_from_github()
    if github_data:
        github_data.setdefault("files", {})
        github_data.setdefault("unsorted_decks", {})
        return github_data
...
def save_data(data):
    try:
        ...
        # Also sync to GitHub if configured (for Streamlit Cloud persistence)
